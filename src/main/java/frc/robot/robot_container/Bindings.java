...
public class Bindings {
    ...
    public void bind(Command command, TriggerBehavior tb, XboxCL button, XboxCL... buttons) {
        if (build)
            throw new IllegalStateException("Cannot bind after build()!");

        Set<XboxCL> buttonSet = new HashSet<>();
        buttonSet.add(button);
        for (XboxCL b : buttons) {
            if (!buttonSet.add(b))
                throw new IllegalStateException("Cannot add duplicate buttons.");
        }
    ...
    public void build() {
        ...
        for (QueueBinding binding : queuedBindings) {
            ...
            if (debounce)
                trigger = trigger.debounce(DEBOUNCE);
            ...
            String buttonOrder = binding.buttons()
                    .stream()
                    .map(Enum::name)
                    .sorted()
                    .collect(Collectors.joining("+"));
            String name = "Controllers/"
                    + this.name
                    + "/"
                    + buttonOrder
                    + " | "
